...
A1 = sort([1:9, 11]); %internal buses, 30 bus system
% A1 = sort([1:5, 7,8]);  %internal buses, 30 bus system
A2 = sort(setdiff(1:nbus, A1)); %external buses
...
% max_min_KL=0;
KL_sorted_A2=cell(3,length(C)*(length(A2)-k2));
optimal_remove=[];
count=1;
for k=1:length(C)
...
%     klmean(i) = 1/2*(mu{i}'*inv(sigma0)*mu{i});
    ...
    if KL_sorted_A2{1,count}(1,1)<-1e-5
        error('KL div. negative')
    end

    if max_min_KL(k2)<KL_sorted_A2{1,count}(1,1)
        max_min_KL(k2)=KL_sorted_A2{1,count}(1,1);
