...
klcov_lines = zeros(numel(loseline),4); %KL divergence and associated line loss
...
%compute the D(f14||fi), want this to be large as possible
D_f14_fi=zeros(numel(loseline),2);
D_fi_f14=zeros(numel(loseline),2);
for i=1:numel(loseline)
    ...
    D_f14_fi(i,1)=KL_compute(sigma{14}, sigma{i}); 
    D_f14_fi(i,2)=i;
    D_fi_f14(i,1)=KL_compute(sigma{i}, sigma{14}); 
    D_fi_f14(i,2)=i;
end
D_f14_fi_sorted=sortrows(D_f14_fi);
D_fi_f14_sorted=sortrows(D_fi_f14);
...
for i=1:numel(loseline)
    ...
    [soln_fault, ybus_fault, J_fault] = get_pf_v4(bus,line2);
...
%             V0 = soln(1:end,2);
            ...
            line2=line;
            line2(linefault,:)=[];
            [soln_sample, ybus_sample, J_sample] = get_pf_v4(bus_sample,line2);
%             V0 = soln(1:end,2);
            theta_sample = soln_sample(1:end,3).*pi/180; %in radians
            ynew=theta_sample;
            dy = ynew'-y';
