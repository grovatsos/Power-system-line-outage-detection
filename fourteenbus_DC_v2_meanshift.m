...
y=theta0;
ADD=0;
A=[14.5 16.3 17 17.7 18.4 19.7]; %Threshold
% A=A(1);
A=4;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
mu = cell(numel(loseline),1);
mu2= cell(numel(loseline),1); %different way to compute mu?
...
%     fij = fij(A1);
    ...
    mu2{i}=theta0_fault{i}-theta0;
end

for pathId=0:(num_samplepaths-1) %pathID start with 0 due to cumulative averaging formula later on
    tao=1; 
    f_stop=0;
    extra_points=0; %counter
    extra_points_threshold=10; %additional points to be plotted after fault detected
...
%         Wmun(i) = subplus(log(mvnpdf(dy(PMU_bus-1),mu{i}(PMU_bus-1)',sigma0)/mvnpdf(dy(PMU_bus-1), mu0(PMU_bus-1), sigma0)));
        Wmun(i) = subplus(log(mvnpdf(dy(PMU_bus-1),mu2{i}(PMU_bus-1)',sigma0)/mvnpdf(dy(PMU_bus-1), mu0(PMU_bus-1), sigma0)));
        ...
        if sum(subplus(Wmun-ones(1, numel(loseline))*A))>0
...
figure;
title(['Single Line Outage', num2str(linefault)])
hold on, grid on;
...
p5_mu=plot(1:tao, Wmunvec(1:tao,linefault)','--b', 'LineWidth', 3);
p2_mu=plot(1:tao, Wmunvec(1:tao,2)','--g', 'LineWidth', 3);
p3_mu=plot(1:tao, Wmunvec(1:tao,3)', '--r', 'LineWidth', 3);
% legend([p5_mu, p2_mu, p3_mu], '5 meanshift','2 meanshift','3 meanshift');

% figure;
% title('Xichen Code 14bus limited PMUs')
% hold on, grid on;
p5=plot(1:tao, Wnvec(1:tao,linefault)','b', 'LineWidth', 3);
p2=plot(1:tao, Wnvec(1:tao,2)','g', 'LineWidth', 3);
p3=plot(1:tao, Wnvec(1:tao,3)','r', 'LineWidth', 3);
l1=legend([p5, p2, p3, p5_mu, p2_mu, p3_mu ], '$W_5[k]$','$W_2[k]$','$W_3[k]$','Meanshift 5','Meanshift 2','Meanshift 3');
...
%Plot all meanshift streams  
figure('position', [20 80 1300 800]);
axes('position', [0.14 0.14 0.8 0.8]);
hold on; box on; grid on;
title(['All streams for Single Line Outage ', num2str(linefault)])
% plot(1:tao, Wnvec(1:tao, :)', '--');
plot(1:tao, Wmunvec(1:tao, :)','-');
% scatter(1:tao, Wmunvec(1:tao,5)','X');
