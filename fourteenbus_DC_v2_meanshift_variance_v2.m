...
%8/20/2015: Performs variance computation the samplings. 

%8/21/2015: Loops through multiple values of threshold (instead of single
%threshold from previous version)
...
ADD=Inf;
threshold=[0.5 1]; %
E_l=zeros(numel(threshold), num_samplepaths);
...
%             Wn_Covshift(i) = (log(mvnpdf(dy, mu0, sigma{i})/mvnpdf(dy, mu0, sigma0)));
            
            Wn_Shewhart(i) = max(log(mvnpdf(dy, mu0, sigma{i})/mvnpdf(dy, mu0, sigma0))); % No more meanshift for this type of sampling of delta theta %...
%             log(mvnpdf(dy,mu{i}',sigma0)/mvnpdf(dy, mu0, sigma0)));
        end
...
%         E_l(i)=temp/numel(Y_k);
    ...
    E_l(i, pathId)=temp/numel(Y_k)
end
end

mu_hat=zeros(1,numel(threshold));
for i=1:numel(threshold) 
    mu_hat(i)=mean(E_l(i,:))
    mean((E_l(i,:)-mu_hat(i)).^2) %variance
end
