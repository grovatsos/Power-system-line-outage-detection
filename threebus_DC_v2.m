...
seed=4;
% PMU_bus=2; %bus 1 is slack. So either have PMU on bus 2, or bus 3
%%%%%%%%%%
...
mu=[0 0]; %mean of theta2 and theta3
Psig = 0.5;
vari = diag([2*Psig^2, 2*Psig^2]); %variance of power injection
sigma0=B*vari*B'; %variance of theta, no outage
sigma1=(B+dB12)*vari*(B+dB12)';
sigma2=(B+dB23)*vari*(B+dB23)';
sigma3=(B+dB13)*vari*(B+dB13)';
...
P1= ynom(3)/X12*sin(-ynom(1)) + ynom(4)/X13*sin(-ynom(2)); %Calculated power generated at bus 1
...
%         X=mvnrnd(mu, sigma0, 1);
        ...
        % Algorithm 1
        Wn(1) = subplus(Wn(1) + log(mvnpdf(dy, mu, sigma1)/mvnpdf(dy, mu, sigma0)));
        Wn(2) = subplus(Wn(2) + log(mvnpdf(dy, mu, sigma2)/mvnpdf(dy, mu, sigma0)));
        Wn(3) = subplus(Wn(3) + log(mvnpdf(dy, mu, sigma3)/mvnpdf(dy, mu, sigma0))); 
        
        % Algorithm 2, takes into account instantaneous change
        Wmun(1) = max(subplus(Wmun(1) + ...
            log(mvnpdf(dy, mu, sigma1)/mvnpdf(dy, mu, sigma0))), ...
            log(mvnpdf(dy,mu1',sigma0)/mvnpdf(dy, mu, sigma0)));
        
        Wmun(2) = max(subplus(Wmun(2) + ...
            log(mvnpdf(dy, mu, sigma2)/mvnpdf(dy, mu, sigma0))), ...
            log(mvnpdf(dy,mu2',sigma0)/mvnpdf(dy, mu, sigma0)));
        
        Wmun(3) = max(subplus(Wmun(3) + ...
            log(mvnpdf(dy, mu, sigma3)/mvnpdf(dy, mu, sigma0))), ...
            log(mvnpdf(dy, mu3',sigma0)/mvnpdf(dy, mu, sigma0)));
