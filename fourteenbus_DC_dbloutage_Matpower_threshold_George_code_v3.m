...
%6/30/2015: Add code to compute zeta, which is used to compute a better
%threshold alpha. See George's notes. 
...
for current_line=1:nline
    tic
    if ismember(current_line, critical_lines_single)
       zeta_theta(current_line)=NaN;
       continue;
    end
...
% end
num_samplepaths=8000;
y=theta0;
ADD=Inf;
%run sample paths
%scale thresholds according to KL divergences
% A_single=(100*KL_single(:,1)/max(KL_single(:,1)))';
A_single=5000*ones(1, numel(loseline_single)); %Threshold
overshoot=zeros(1, num_samplepaths);
...
% gamma=1/FA_rate;
% gamma=10000;
% alpha=log(gamma)+log(numel(loseline_single)); %loseline_single=19
% A_single=alpha-log(1./((KL_single_limitedPMUs(:,1))'.*zeta_theta.^2));
...
%                 continue
%             end
            Wn_single = Wn_single + log(mvnpdf(dy(PMU_bus-1), mu0(PMU_bus-1), sigma_single{current_line})/mvnpdf(dy(PMU_bus-1), mu0(PMU_bus-1), sigma0));
%         end
        ...
        if Wn_single-A_single(current_line)>0
...
%             disp(['single line outage streams ', num2str(f_single), ' crossed the treshold ', num2str(A_single(f_single))]);
            ...
            f_stop==1;
            overshoot(pathId)=Wn_single-A_single(current_line);
            break; %needed so tao_single doesn't increase  
        end
        %Update the stopping variable till we actually stop
        tao_single=tao_single+1;
        
        if tao_single>1000000
            error('timed out')
        end
    end
end
disp(['line finished ', num2str(current_line)]);
zeta_theta(current_line)=mean(exp(-(overshoot)))
time(current_line)=toc

filename=['zeta_threshold_line', num2str(current_line)];
save(filename)
end
...
% p1=plot(1:tao_single, Wnvec_single(1:tao_single, :)','--b', 'LineWidth', 2, 'Marker', '*', 'Markersize', 6);
