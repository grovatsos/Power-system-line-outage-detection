...
%5/9/2014: the area 2 slack bus does not have to be the 1st element of remove.

%5/27/2014:Automatically loops through number of PMUs allocated to Area 2 and saves all data
%          Got rid of intermidiary results due to selecting optimal slack bus, only final result is stored  
clear all
close all
% clc
...
max_min_KL=zeros(1,length(A2)-1);
best_k=zeros(1,length(A2)-1);
for k2=1:length(A2)-1
% k2=1; 
%%%%%%
...
%     klmean(i) = 1/2*(mu{i}'*inv(sigma0)*mu{i});
    ...
    if max_min_KL(k2)<max_min_KL_k
        max_min_KL(k2)=max_min_KL_k;
        best_k(k2)=k;
    end
...
%        disp(sprintf(['count=' num2str(count)]));
       disp(sprintf(['k2=' num2str(k2)]));
       disp(sprintf(['k=' num2str(k)]));
    end     
    end
    
    KL_sorted_A2{1,k}=best_sorted_KL; %sort KL divergnce from min to max as well as their associated lines
    KL_sorted_A2{2,k}=optimal_slack;
    if length(remove)>1
        KL_sorted_A2{3,k}=setdiff(optimal_remove, optimal_slack);
    end
end
str=['KL_divergence_',num2str(k2),'PMU.mat'];
save(str)

disp(sprintf(['\nnum PMUs allocated for Area 2= ' num2str(k2)]));
disp(sprintf(['min KL divergence optimal placement of PMUs for Area 2 = ' num2str(max_min_KL(k2)), '\nslack bus = ' num2str(optimal_slack), '\nother buses without PMUs = ', num2str(optimal_remove)]));
disp(sprintf(['PMUs at buses ' num2str(setdiff(A2,[optimal_remove optimal_slack]))]));
end
