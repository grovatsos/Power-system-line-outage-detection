%
%9/14/2015: Test whether critical line outages can be detected by using generalized
%inverse for jacobian.
...
bus=mpc.bus;
bus(8,2:end)=bus(2,2:end); %change bus 8 to be a generator bus 
line=mpc.branch;
baseMVA = mpc.baseMVA;
nbus = size(bus,1);
nline = size(line,1);
...
for i = 1:length(loseline_dbl)    
    ...
    Ji = J0 + dJ_first+ dJ_second; %H matrix in paper
    if rank(Ji) < size((Ji),1)
...
%         disp('singular Jacobian A1')
    ...
    sigma_dbl{loseline_dbl(i,1),loseline_dbl(i,2)} = Ji\(2*Psig^2*eye(nbus-1))/(Ji'); %make it symmetric to avoid numerical rounding issues
...
A_single=1000*ones(1, numel(loseline_single)); %Threshold
...
% A_single=alpha-log(1./KL_single_limitedPMUs(:,1))';
...
A_dbl=1000*ones(1, length(loseline_dbl)); %Threshold
