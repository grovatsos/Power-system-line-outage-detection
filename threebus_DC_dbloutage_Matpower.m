...
for pathId=1:num_samplepaths 
    ...
    while f_stop==0
        P=bus(2:end,3); %Negative signs are to match earlier version of code. Note that Pload and Qload may become negative meaning bus is generating power. 
        Q=bus(2:end,4);
        ...
        if tao_single < faulttime %no fault
            bus_sample=bus;
            bus_sample(2:end,3)=P_sample;
            bus_sample(2:end,4)=Q_sample;
            mpc.bus=bus_sample;
            results = runpf(mpc, mpopt);
