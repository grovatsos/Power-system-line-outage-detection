...
%test and Cusum tests. Compute ADD. Also, the meanshift is computed
%from nonlinear power flow but the samples of theta are taken directly from
%normal distribution. 

%9/17/2015: Compares the KL divergences computed using DC Jacobian and AC
%jacobian.
...
fullJac=true;
J_full=makeJac(mpc, fullJac);
J_full=full(J_full(2:14,2:14));
...
figure;
plot(Wnvec_Covshift(:,linefault_single), '*');
title('Covariance shift only')

figure;
plot(Wnvec_Meanshift(:,linefault_single), '*');
title('meanshift only');

% figure;
% plot(Wnvec_Meanshift, '*');
% title('meanshift only');

% threshold=2; %only one threshold 
% temp=0;
% count=0;
%     
%     for j=1:numel(Y_k)
...
%     end
%     E_l(pathId)=temp/numel(Y_k)
% end
% 
% mu_hat=mean(E_l)
% mean((E_l-mu_hat).^2) %variance
% 
