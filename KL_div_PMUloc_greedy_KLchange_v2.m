...
%10/1/2014 Fix the slack bus when running optimization. This provides a benchmark for seeing if KL div are increase/decrease due 
%PMU increase and not slack bus location change
...
for k=1:k2 %add each PMU successively
    ...
    optimal_remove_k=[];
    optimal_slack_k=[];
    for l=2:length(A2)         %exhaustively search next PMU location by computing KL divergence
