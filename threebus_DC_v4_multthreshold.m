...
y=theta0;
ADD=0;
%KL1=3.6976, KL2=1.7739, KL3=6.4208, KL divergences for ALL PMUs
A_1=[57.58 27.63 100]; %set threshold for each line
%KL1=2.9, KL2=0.52, KL3=0.06 KL divergences for PMU on bus 2 only
A_2=[100 18 2.06]; %set threshold for each line
%KL1=0.98, KL2=1.45, KL3=0.21, KL divergences for PMU on bus 3 only
A_3=[67.58 100 14.4]; %set threshold for each line

%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%B0 and sigma0 are modified according to buses without PMUs
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
remove_PMU_bus=[3]; %buses that do not have PMUs, bus 1 is slack, so remove PMU can only remove from 2 or 3
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if remove_PMU_bus== 2
        A=A_3;
elseif remove_PMU_bus==3
        A=A_2;
else
        A=A_1;
end
...
% hold on, grid on;
p1=plot(1:tao, Wnvec(1:tao,1)','--b', 'LineWidth', 2, 'Marker', '*', 'Markersize', 6);
...
l1=legend([p1, p2, p3], '$W_{(1,2)}$','$W_{(1,3)}$','$W_{(2,3)}$');
