...
%11/18/2014: Adapted from 3bus to 30bus system. 
...
%%%%%%%%%%%
% 
% %%This is for removing reactive power limits
% bus(:,11)=100*ones(size(bus(:,11))) %Qmax
% bus(:,12)=-100*ones(size(bus(:,12))) %Qmin
...
sigma0 = B0*(2*Psig^2*eye(length(A1)-1))*B0'; %M*Sigma*M' in paper, this is all A1 subsystem

loseline_single = 1:nline;
temp = find(ismember(line(:,1),A2)|ismember(line(:,2),A2)); %a line that has node in A2 (the KL divergences and sample paths are computed for lines in A1 only)
loseline_single(unique([temp'])) = []; %can't lose lines with a node in A2
...
for i = 1:numel(loseline_single)
    ...
    sigma_single{i} = Ji\(2*Psig^2*eye(length(A1)-1))/(Ji'); %make it symmetric to avoid numerical rounding issues
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Double Line Outage, all combinations of loseline_first and loseline_second
loseline_dbl=[5 6; 1 4; 1 9]; %each row specifies the pair of line to be outaged  
loseline_dbl=combnk(loseline_single, 2);


% if all(ismember([loseline_first loseline_second],critical_lines ==0)) %cannot lose these lines, otherwise islanding occurs
%     error('choose different double line outages')
% end

B_dbl = cell(numel(loseline_single),numel(loseline_single));
sigma_dbl = cell(numel(loseline_single),numel(loseline_single));
KL_dbl = zeros(length(loseline_dbl),3); %KL divergence for dbl outage

%Compute KL divergence for a double line outage
for i = 1:length(loseline_dbl)    
    %first line
    from = line(loseline_dbl(i,1),1);
    to = line(loseline_dbl(i,1),2);
    xij = line(loseline_dbl(i,1),4);
    fij = zeros(nbus,1);
    fij(from) = 1;
    fij(to) = -1;
    fij = fij(A1);
    fij(1) = []; %remove slack bus
    dJ_first = -1/xij*(fij*fij');
    
    %second line
    from = line(loseline_dbl(i,2),1);
    to = line(loseline_dbl(i,2),2);
    xij = line(loseline_dbl(i,2),4);
    fij = zeros(nbus,1);
    fij(from) = 1;
    fij(to) = -1;
    fij = fij(A1);
    fij(1) = []; %remove slack bus
    dJ_second = -1/xij*(fij*fij');
    
    Ji = J11 + J12_tilde + dJ_first+ dJ_second; %H matrix in paper
    if rank(Ji) < size((Ji),1)
        disp(['critical double outage lines ' num2str(loseline_dbl(i,1)), ' and ' num2str(loseline_dbl(i,2))]);
        disp(['line ' num2str(loseline_dbl(i,1)), ' has buses from ', num2str(line(loseline_dbl(i,1),1)), ' to ' num2str(line(loseline_dbl(i,1),2))]);
        disp(['line ' num2str(loseline_dbl(i,2)), ' has buses from ', num2str(line(loseline_dbl(i,2),1)), ' to ' num2str(line(loseline_dbl(i,2),2)), sprintf('\n')]);
%         disp('singular Jacobian A1')
        B_dbl{loseline_dbl(i,1),loseline_dbl(i,2)} = NaN;   B_dbl{loseline_dbl(i,2),loseline_dbl(i,1)} = NaN;
        sigma_dbl{loseline_dbl(i,1),loseline_dbl(i,2)} = NaN;   sigma_dbl{loseline_dbl(i,2),loseline_dbl(i,1)} = NaN;
        KL_dbl(i,1) = NaN; 
        KL_dbl(i,2) = loseline_dbl(i,1); 
        KL_dbl(i,3) = loseline_dbl(i,2); 

        continue
    end
       
    B_dbl{loseline_dbl(i,1),loseline_dbl(i,2)} = inv(Ji);   B_dbl{loseline_dbl(i,2),loseline_dbl(i,1)} = inv(Ji);
    sigma_dbl{loseline_dbl(i,1),loseline_dbl(i,2)} = Ji\(2*Psig^2*eye(length(A1)-1))/(Ji'); %make it symmetric to avoid numerical rounding issues
    ...
    KL_dbl(i,:) = [1/2 *(trace(sigma0\sigma_dbl{loseline_dbl(i,1),loseline_dbl(i,2)}) - ...
        length(sigma0) + log(det(sigma0/sigma_dbl{loseline_dbl(i,1),loseline_dbl(i,2)}))); loseline_dbl(i,1); loseline_dbl(i,2)];
    KL_dbl(i,2) = loseline_dbl(i,1); 
    KL_dbl(i,3) = loseline_dbl(i,2);  
   
end
temp=find(isnan(KL_dbl)); %lines that if lost, creates islanding situations
critical_lines_dbl=KL_dbl(temp, 2:3);
KL_dbl_sorted=sortrows(KL_dbl,1);
%disp(['min KL divergence dbl outage = ' num2str(KL_dbl_sorted(1,1))]);
...
for i = 1:numel(loseline_single)
    ...
    Bi(remove_PMU_bus-1,:)=[];
    B_single{i} = Bi;
    sigma_single{i} = Bi*(2*Psig^2*eye(length(A1)-1))*Bi'; %make it symmetric to avoid numerical rounding issues
    aa = triu(sigma_single{i});
    sigma_single{i} = aa + triu(aa,1)';
...
for i = 1:length(loseline_dbl)
    ...
    sigma_dbl{loseline_dbl(i,1),loseline_dbl(i,2)} = Bi*(2*Psig^2*eye(length(A1)-1))*Bi'; %make it symmetric to avoid numerical rounding issues
    aa = triu(sigma_dbl{loseline_dbl(i,1),loseline_dbl(i,2)});
    sigma_dbl{loseline_dbl(i,1),loseline_dbl(i,2)} = aa + triu(aa,1)';
    sigma_dbl{loseline_dbl(i,2),loseline_dbl(i,1)}=sigma_dbl{loseline_dbl(i,1),loseline_dbl(i,2)};
...
linefault_single=3; %line 3 8 12 seems to work
...
for i=1:numel(loseline_single)
    ...
    [soln_fault, ybus_fault, J_fault] = get_pf(bus,line_temp);
...
%             V0 = soln(1:end,2);
            ...
            [soln_sample, ybus_sample, J_sample] = get_pf(bus_sample,line_temp);
%             V0 = soln(1:end,2);
            theta_sample = soln_sample(1:end,3).*pi/180; %in radians
            ynew=theta_sample;
            dy = ynew'-y';
        ...
        if sum(subplus(Wn_single-A_single))>0 %(sum(subplus(Wn_single-A_single))>0) ||(sum(subplus(Wn_dbl-A_dbl))>0)
...
%               find(temp(1)==(Wn_dbl-A_dbl))
                ...
                warning('misdetection of single line fault')
            end
            if pathId==1 
                ADD = tao_single;
            else
...
p4=plot(1:tao_single, Wnvec_dbl(1:tao_single,max_dbl)','--m', 'LineWidth', 2,'Marker', 's','Markersize', 6);
l1=legend([p1, p2, p3, p4], '$W_{linefault single}$','$W_{5}$','$W_{2}$', '$W_{dbl}$');
...
% seed=4;
linefault_dbl=[3 8]; %choose the two lines that are outaged
...
p2=plot(1:tao_dbl, Wnvec_dbl(1:tao_dbl, 1)','--g', 'LineWidth', 2,'Marker', 'o','Markersize', 6);
p3=plot(1:tao_dbl, Wnvec_single(1:tao_dbl, linefault_dbl(1))','--r', 'LineWidth', 2,'Marker', 's','Markersize', 6);
p4=plot(1:tao_dbl, Wnvec_single(1:tao_dbl, linefault_dbl(2))','--m', 'LineWidth', 2,'Marker', 's','Markersize', 6);
% p5=plot(1:tao_dbl, Wnvec_single(1:tao_dbl, max_single)','--k', 'LineWidth', 2,'Marker', 's','Markersize', 6);
l1=legend([p1, p2, p3, p4], '$W_{(5,6)}$','$W_{(1,2)}$','$W_{(5)}$', '$W_{(6)}$');
...
%given single line outage, single line outage streams growth
...
%given single line outage, double line outage streams growth
...
for i=1:numel(loseline_single)
    if mod(i, 2)==0
        disp(['E_single_dbl ', num2str(i), ' out of ', num2str(numel(loseline_single))]);
    end
    if ismember(loseline_single(i), critical_lines_single)
        E_single_dbl(i,:)=NaN;
        continue
...
for i=1:length(loseline_dbl)
    if mod(i, 30)==0
        disp(['E_dbl_single ', num2str(i), ' out of ', num2str(length(loseline_dbl))]);
    end
    if ismember(loseline_dbl(i,:), critical_lines_dbl, 'rows')
        E_dbl_single(i,:)=NaN;
        continue
...
for i=1:length(loseline_dbl)
    if mod(i, 2)==0
        disp(['E_dbl_dbl ', num2str(i), ' out of ', num2str(length(loseline_dbl))]);
    end
    if ismember(loseline_dbl(i,:), critical_lines_dbl, 'rows')
        E_dbl_dbl(i,:)=NaN;
        continue
...
percent=count/length(loseline_dbl) %about 21%
