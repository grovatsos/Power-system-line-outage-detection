...
%9/14/2015: testing islanding detection
...
%     end
    ...
    B_single{i} = pinv(Ji); %Compute each M corresponding to each line outage in A1;
    sigma{i} = B_single{i}*(2*Psig^2*eye(nbus-1))*B_single{i}'; %make it symmetric to avoid numerical rounding issues
    aa = triu(sigma{i});
